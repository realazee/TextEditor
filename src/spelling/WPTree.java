...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		Queue<WPTreeNode> nodeQueue = new LinkedList<WPTreeNode>();
		nodeQueue.add(this.root);
		LinkedList<String> visitedWords = new LinkedList<String>(); 
		visitedWords.add(word1);


		while(!nodeQueue.isEmpty()) {
			WPTreeNode currentNode = nodeQueue.remove();

			List<String> oneAwayFromCurr = nw.distanceOne(currentNode.getWord(), true);
			for(String n : oneAwayFromCurr) {
				if(!visitedWords.contains(n)) {
					WPTreeNode childOfCurrNode = currentNode.addChild(n);
					nodeQueue.add(childOfCurrNode);
					visitedWords.add(n);
					if(n.equals(word2)) { //path has been found, time to return! 
						return childOfCurrNode.buildPathToRoot();
